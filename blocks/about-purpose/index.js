...
registerBlockType(metadata.name, {
  ...
  edit: ({ attributes, setAttributes }) => {
    const { eyebrow, titleLine1, titleLine2, badgeLabel, badgeText, paragraphs } = attributes;
    ...
    const addParagraph = () => setAttributes({ paragraphs: [...paragraphs, 'New paragraph.'] });
    const removeParagraph = (index) => setAttributes({ paragraphs: paragraphs.filter((_, i) => i !== index) });
          ...
          <div>
            ...
            <div className="inline-block px-4 py-2 bg-primary/5 border-l-4 border-secondary mt-4">
              <p className="text-primary font-bold text-sm uppercase tracking-wider">
                <RichText tagName="span" value={badgeLabel} onChange={(value) => setAttributes({ badgeLabel: value })} allowedFormats={[]} />{' '}
                <RichText tagName="span" className="text-on-surface-variant font-medium normal-case" value={badgeText} onChange={(value) => setAttributes({ badgeText: value })} allowedFormats={[]} />
              </p>
            </div>
          </div>
          <div className="space-y-6 text-on-surface-variant text-body-lg leading-relaxed">
            {paragraphs.map((paragraph, index) => (
              <div key={index} className="relative group">
                <RichText tagName="p" value={paragraph} onChange={(value) => updateParagraph(index, value)} allowedFormats={['core/bold', 'core/italic']} />
                <Button className="absolute -top-2 -right-2 opacity-0 group-hover:opacity-100" isDestructive isSmall onClick={() => removeParagraph(index)}>×</Button>
              </div>
            ))}
