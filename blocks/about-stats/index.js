        ...
        <div className="container-custom">
          <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-6 gap-[2px] bg-outline-variant/20 shadow-sm overflow-hidden">
            {items.map((item, index) => (
              <div
                key={index}
                className="bg-white p-10 flex flex-col justify-between aspect-square hover:bg-surface-container-low transition-colors duration-500"
              >
                <RichText
                  tagName="span"
                ...
                <div className="space-y-2">
                  ...
                  <RichText
                    tagName="span"
                    className="block text-on-surface-variant text-[10px] uppercase tracking-widest gap-2"
                    value={item.sublabel}
                    onChange={(value) => updateItem(index, "sublabel", value)}
                    allowedFormats={[]}
                  />
                  <ToggleControl
                    label="Highlight (secondary color)"
