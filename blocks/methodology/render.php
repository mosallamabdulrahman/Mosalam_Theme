    ...
    <div class="js-methodology-tabs w-full overflow-x-auto hide-scrollbar mb-12 border-b border-black/10 cursor-grab active:cursor-grabbing select-none" data-animate="fade-up">
      <div class="flex w-max min-w-full">
        <?php foreach ($steps as $index => $step) : ?>
          <button
            type="button"
            class="js-methodology-tab-btn min-w-[180px] md:min-w-[200px] flex-1 text-center pb-4 text-h4 transition-all duration-300 relative <?php echo 0 === $index ? 'text-primary' : 'text-on-surface-variant/50 hover:text-primary/70'; ?>"
      ...
      <?php foreach ($steps as $index => $step) : ?>
        <div class="js-methodology-panel flex flex-col md:flex-row min-h-[400px] <?php echo 0 === $index ? '' : 'hidden'; ?>" data-step-index="<?php echo (int) $index; ?>">
          <div class="w-full md:w-1/2 p-12 flex flex-col justify-center">
            ...
            <h3 class="text-h2 text-primary mb-6"><?php echo esc_html($step['title']); ?></h3>
          ...
          <div class="w-full md:w-1/2 h-64 md:h-auto relative overflow-hidden">
            <img src="<?php echo esc_url(MOSALAM_THEME_URI . '/assets/images/' . $step['image']); ?>" alt="<?php echo esc_attr($step['title']); ?>" class="w-full h-full object-cover" referrerpolicy="no-referrer" loading="lazy">
