    ...
    const updateLine = (itemIndex, lineIndex, value) => {
      const next = contactItems.map((item, i) => {
        ...
        const lines = item.lines.map((line, j) =>
          j === lineIndex ? { ...line, text: value } : line,
        );
        return { ...item, lines };
      });
      setAttributes({ contactItems: next });
        ...
        <div className="container-custom grid grid-cols-1 lg:grid-cols-12 gap-16 lg:gap-24">
                  ...
                  <div className="flex-1">
                    ...
                    {item.lines.map((line, lineIndex) => (
                      <RichText
                        ...
                        className={
                          line.href
                            ? "text-body text-secondary block"
                            : "text-body text-on-surface-variant"
                        }
                        value={line.text}
                        onChange={(value) =>
                          updateLine(index, lineIndex, value)
                        }
                        allowedFormats={[]}
                      />
                    ))}
                    {item.note && (
                      <RichText
                        tagName="p"
                        className="text-sm text-on-surface-variant/70 mt-1"
                        value={item.note}
                        onChange={(value) => updateItem(index, "note", value)}
                        allowedFormats={[]}
                      />
                    )}
                  </div>
          ...
          <div className="lg:col-span-7 border border-black/5">
            <div className="bg-white p-10 md:p-16 shadow-xl relative overflow-hidden">
              ...
              <RichText
                tagName="h3"
                className="text-h3 text-primary mb-8"
                value={formTitle}
              ...
              <div className="space-y-8">
                ...
                <div className="flex flex-col gap-2">
                  <RichText
                    ...
                    className="text-sm font-bold text-primary uppercase tracking-wider"
                    value={orgLabel}
                    onChange={(value) => setAttributes({ orgLabel: value })}
                    allowedFormats={[]}
                  />
                  <TextControl
                    value={orgPlaceholder}
                    onChange={(value) =>
                      setAttributes({ orgPlaceholder: value })
                    }
                    help="Placeholder text"
                    __next40pxDefaultSize
                    __nextHasNoMarginBottom
                ...
                <div className="flex flex-col gap-2">
                  <RichText
                    ...
                    className="text-sm font-bold text-primary uppercase tracking-wider"
                    value={inquiryLabel}
                    onChange={(value) => setAttributes({ inquiryLabel: value })}
                    allowedFormats={[]}
                  />
